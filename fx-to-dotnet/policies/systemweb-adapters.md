...
5. **Stabilize with Build Fix** — Invoke `speckit.fx-to-dotnet.fix` to resolve compilation errors introduced by the package swap. The command will iterate `dotnet build` → diagnose → fix until the project builds.
