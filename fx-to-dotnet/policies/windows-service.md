...
6. **Stabilize with Build Fix** — Invoke `speckit.fx-to-dotnet.fix` to resolve remaining compilation errors. The command will iterate `dotnet build` → diagnose → fix until the project builds.
...
- **EF6 in services**: If the Windows Service uses Entity Framework 6, follow the `policies/ef6-retention.md` policy — retain EF6, do not replace with EF Core during migration.
